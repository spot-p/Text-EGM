...
def main(args):
    ...
    np.save('../faulty_data/train_intra_2.npy', train)
    np.save('../faulty_data/val_intra_2.npy', val)
    np.save('../faulty_data/test_intra_2.npy', test)

if __name__ == '__main__':
    args = get_args()
    #this is custom from me, jake cumberland
    #here's why we need to do it this way on windows 11
    #basically, the shell script passes the folder name as the argument right?
    #it's supposed to be interpreted as a string, but for some reason, it's still wrapped as a namespace object instead with the string inside it
    #just doing args = args.path gets it as a string like it's supposed to be
    #args = args.path
    main(args)
